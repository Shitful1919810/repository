...
<html>
  <head>
    ...
    <script defer="defer">
      ...
      function checkState() {
        ...
        xmlhttp.onreadystatechange = function () {
          if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            var stateInfo = xmlhttp.responseText;
            var para = stateInfo.split(" ");
            var ledInt = parseInt(para[0]);
            var dhtInt = parseInt(para[1]);
            var nData = parseInt(para[2]);
            document.getElementById("redLEDInterval").value = ledInt;
            document.getElementById("rangeInterval").value = dhtInt;
            document.getElementById("rangeNData").value = nData;
            changeLEDInterval();
            changeInterval();
            changeNData();
          }
        };
        xmlhttp.open("GET", "sta", true);
        xmlhttp.send();
      }
    </script>
  </head>
  <body>
    ...
    <input type="button" value="同步状态" onclick="checkState()" /><br />
